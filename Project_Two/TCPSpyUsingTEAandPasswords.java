...
public class TCPSpyUsingTEAandPasswords {
    ...
    public static void main (String args[]) throws IOException {
        ...
        try{
            ...
            // get the symmetric key from the client
            System.out.println("Enter symmetric key for TEA (taking first sixteen bytes):");
            String clientKeyFull = scanClient.nextLine();
            
            TEA tea;
            
            int len = clientKeyFull.length();

            byte[] clientKeyByte = clientKeyFull.getBytes();

            // Use the symmetric key to initiate TEA

            tea = new TEA(clientKeyByte);

            // encrypt client symmetric key 
            byte[] clientKeyEncrypted = tea.encrypt(clientKeyByte);
            out.writeInt(clientKeyEncrypted.length);
            out.write(clientKeyEncrypted);
